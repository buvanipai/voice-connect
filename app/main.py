...
async def transcribe_webhook(request: Request):
    ...
    if ai_response_obj.intent != "ERROR":
        try:
            # Update with new intent and timestamp
            updated_data["last_intent"] = ai_response_obj.intent
            updated_data["last_interaction"] = datetime.datetime.now().isoformat()
            
            # Merge newly extracted entities (overwrite only if new data is non-empty)
            if ai_response_obj.entities:
                for key, value in ai_response_obj.entities.items():
                    # Only save non-empty string values, skip None/empty/null
                    if value is not None and isinstance(value, str) and value.strip():
                        updated_data[key] = value.strip()
                    # Log if we're skipping a null or empty value
                    if value is None:
                        print(f"[{call_sid}] Skipping null entity: {key}")
            
            profile_service.update_profile(caller_number, updated_data)
            filtered_entities = {k: v for k, v in ai_response_obj.entities.items() if v}
            print(f"[{call_sid}] Profile updated. New entities: {filtered_entities}")
            print(f"[{call_sid}] Full profile now: {updated_data}")
        except Exception as e:
